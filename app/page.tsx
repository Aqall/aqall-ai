          ...
          >
            ...
            >
              <Button variant="hero" size="xl" asChild className="group relative overflow-hidden">
                <Link href="/auth?mode=signup" className="flex items-center gap-3">
            ...
            <div className="glass-strong rounded-3xl p-2 shadow-elevated">
              <div className="bg-card rounded-2xl border border-border/50 overflow-hidden">
                ...
                <div className="p-6 lg:p-8 space-y-6 bg-gradient-to-b from-muted/20 to-transparent">
                  {/* User message */}
                  <motion.div 
                    className={`flex ${language === 'ar' ? 'flex-row-reverse' : 'flex-row'} justify-end gap-4`}
                    initial={{ opacity: 0, x: language === 'ar' ? -30 : 30 }}
                    animate={{ opacity: 1, x: 0 }}
                    transition={{ delay: 0.8, duration: 0.5 }}
                  >
                    <div className={`bg-background rounded-2xl px-5 py-4 shadow-card max-w-md border border-border/50 ${language === 'ar' ? 'text-right' : 'text-left'}`}>
